package com.myflashlab.air.extensions.admob
{
   ...
   public class AdMobApi extends EventDispatcher
   {
      ...
      private function onStatus(event:StatusEvent) : void
      {
         ...
         switch(event.code)
         {
            ...
            case "onAdFailed":
               ...
               dispatchEvent(new AdMobEvents("onAdFailed",_loc2_[0],_loc2_[1],0,0,_loc2_[2]));
               break;
            case "onAdLeftApp":
               if(OverrideClass["os"] != OverrideClass["ANDROID"] && event.level == "banner")
               {
               ...
               break;
            case "onAdLoaded":
               if(event.level == "banner" && OverrideClass["os"] == OverrideClass["ANDROID"])
               {
      ...
      public function get banner() : ApiBannerAds
      {
         return _banner;
      }
      
      public function get interstitial() : ApiInterstitialAds
      {
         return _interstitial;
      }
      
      public function get rewardedVideo() : ApiRewardedVideo
      {
